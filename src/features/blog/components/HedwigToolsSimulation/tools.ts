...
export const HEDWIG_TOOLS: readonly HedwigTool[] = [
  ...
  },
  {
    ...
    kind: 'slack-builder',
    events: [
      { label: 'Basics entered', detail: 'A fictional bot name and avatar are chosen for the draft.' },
      { label: 'Personality written', detail: 'A plain-language purpose is paired with tone choices.' },
      { label: 'Knowledge connected', detail: 'Demo document spaces toggle on with sync freshness shown.' },
      { label: 'Channels selected', detail: 'Fictional channels mark where the draft bot would live.' },
      { label: 'Test chat simulated', detail: 'Replies are simulated and a request log labels every call as such.' },
      { label: 'Review summarized', detail: 'Name, tone, knowledge, and channels are listed for an admin approver.' },
