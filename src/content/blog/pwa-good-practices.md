...
Text instructions for "tap Share, then scroll, then tap Add to Home Screen" are exactly the kind of thing people skim and get wrong. Short videos work far better, and they turned out to be cheap to produce. I use two kinds.

### The illustrated animation pipeline

The first kind is a fully synthetic, pre-rendered animation — no phone involved. It is generated by one Node script (`npm run generate:install-videos`) and is completely reproducible:
...
Raw phone recordings are unusable as-is — mine was a 24 MB HEVC file — so a second script (`optimize-install-recording.mjs`) turns any capture into web assets:
